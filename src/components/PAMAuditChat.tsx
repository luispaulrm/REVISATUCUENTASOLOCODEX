...
export default function PAMAuditChat({ pamContext }: PAMAuditChatProps) {
  ...
  const contextSnapshot = useMemo(() => pamContext || {}, [pamContext]);
    ...
    try {
      const response = await fetch('/api/pam/chat', {
        ...
        body: JSON.stringify({
          ...
          forceOpenAI,
          pamJson: contextSnapshot
        })
      });

      if (!response.ok || !response.body) {
        throw new Error(`Error ${response.status} en /api/pam/chat`);
      }
      ...
      setHistory((prev) => [
        ...prev,
        {
          id: entryId,
          question: askedQuestion,
          answer: `Error consultando asistente PAM: ${err?.message || 'desconocido'}`,
          images: askedImages
        }
      ]);
      setStreamingAnswer('');
    ...
    <section className="mt-4 border border-slate-200 bg-white rounded-2xl overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200 bg-slate-50">
        <h3 className="text-xs font-black uppercase tracking-wider text-slate-800">
          Asistente rapido PAM (OpenAI)
        </h3>
        <p className="text-[11px] text-slate-500 mt-1">
          Pregunta por folios, prestadores, copagos, bonificaciones, codigos y cuadre del PAM.
        </p>
        <div className="mt-2">
          <button
      ...
      <div ref={scrollRef} className="max-h-[320px] overflow-y-auto px-4 py-3 space-y-3 bg-slate-50/30">
        {history.length === 0 && !streamingAnswer && (
          <div className="text-xs text-slate-400">
            Puedes escribir, copiar/pegar texto o pegar imágenes con Ctrl+V.
          </div>
        )}
      ...
      <div className="px-3 py-3 border-t border-slate-200 bg-white">
        ...
        <div className="flex items-end gap-2 border border-slate-200 rounded-xl px-2 py-2 bg-slate-50">
          ...
          <textarea
            ...
            placeholder="Pregunta sobre el PAM. Puedes pegar imágenes con Ctrl+V."
